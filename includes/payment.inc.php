...
if(isset($_POST['pay_but']) && isset($_SESSION['userId'])) {
    ...
    // Use the real IDs pass_detail.inc.php created, not a guessed range —
    // fall back to the single pass_id only if pass_ids somehow isn't set.
    $pass_ids = isset($_SESSION['pass_ids']) ? $_SESSION['pass_ids'] : array($_SESSION['pass_id']);
    $type = $_SESSION['type'];
